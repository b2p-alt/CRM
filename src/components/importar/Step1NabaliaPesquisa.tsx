...
export default function Step1NabaliaPesquisa({ onDone }: {
  ...
  async function handleSearch(e: React.FormEvent) {
    e.preventDefault();
    if (!codPostal && voltageCodes.length === 0 && !nif) return;
    setLoading(true);
    setError("");
    setRows([]);
    setSelected(new Set());
    ...
    try {
      const res = await fetch("/api/admin/nabalia-cpe", {
        ...
        body: JSON.stringify({ codPostal, codPostalAte, voltageCode: voltageCodes, nipc: nif }),
      });
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Selecione o distrito e pesquise por código postal, NIF e/ou nível de tensão.</p>
      </div>
      ...
      <form onSubmit={handleSearch} className="flex flex-wrap gap-4 items-end">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">NIF da empresa</label>
          <input
            type="text"
            placeholder="ex: PT501442600"
            value={nif}
        ...
        <div className="flex flex-col gap-1">
          <button
            type="submit"
            disabled={loading || (!codPostal && voltageCodes.length === 0 && !nif)}
