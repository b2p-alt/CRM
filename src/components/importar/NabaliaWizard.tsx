...
import Step1NabaliaPesquisa from "./Step1NabaliaPesquisa";
...
const STEPS = ["Pesquisa CPE", "Revisão", "Duplicados", "Contactos", "Importar"];

export default function NabaliaWizard() {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        {step === 0 && (
          <Step1NabaliaPesquisa
            onDone={(records, distrito) => { patch({ records, distrito }); setStep(1); }}
          />
        )}
        {step === 1 && state.records && (
          <Step3Review
        ...
        {step === 2 && state.records && (
          <Step4Duplicados
            records={state.records}
            onDone={(existingNifs, existingCpes) => { patch({ existingNifs, existingCpes }); setStep(3); }}
            onBack={() => setStep(1)}
          />
        )}
        {step === 3 && state.records && (
          <Step4bEnriquecimento
            records={state.records}
            onDone={(enrichResults) => { patch({ enrichResults }); setStep(4); }}
            onSkip={() => { patch({ enrichResults: {} }); setStep(4); }}
            onBack={() => setStep(2)}
          />
        )}
        {step === 4 && state.records && state.distrito && (
          <Step5Confirmar
            ...
            onBack={() => setStep(3)}
            apiEndpoint="/api/admin/nabalia-cpe/confirmar"
            novaImportacaoHref="/admin/nabalia-cpe"
            streaming={true}
          />
        )}
      </div>
